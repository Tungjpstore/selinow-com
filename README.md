...
`platform:route-preflight` là checkpoint staging-only, read-only: nó dùng riêng `CLOUDFLARE_ROUTE_AUDIT_API_TOKEN`, pin Wrangler vào account đã khai báo, đối chiếu live D1 name+UUID và gọi đúng một Worker Routes `GET`; token và raw route response không được in hoặc truyền vào child process. Checkpoint này không thay thế full `platform:doctor`, vốn vẫn cần cả `CLOUDFLARE_PLATFORM_API_TOKEN` và `CLOUDFLARE_ROUTE_AUDIT_API_TOKEN` trước khi chạy backup thật, migration hoặc deploy staging. Source/local chain hiện tới `0052`; staging ledger được chấp nhận vẫn dừng ở `0028`, nên đúng 24 migration `0029`-`0052` còn pending. Read-only evidence hiện tại xác nhận staging D1 identity và `MEDIA` đang private, nhưng phải kiểm tra lại route inventory, D1 identity và `MEDIA` ngay trước mọi mutation; backup report-v2 của đúng staging D1 cũng phải mới không quá 60 phút. Xem thứ tự mutation có guard tại `docs/RELEASE.md`; không suy diễn deploy/migration đã xảy ra từ dry-run.
...
The gate creates disposable local D1/KV state plus a temporary Wrangler config and mode-0600 `.dev.vars` file, applies the current numbered migrations through `0052`, starts Astro in background mode with remote bindings disabled, requests a local-only magic link through the login form, and follows only the visible `mở liên kết đăng nhập` action. Its contract enumerates 14 authenticated seller/admin routes at desktop and mobile widths with screenshot, overflow and axe WCAG A/AA checks. It rejects remote origins and alternate Playwright configs, passes only an explicit local child environment, never exports cookies, storage state, hrefs or magic-link tokens, never submits checkout, and never uses staging/production resources. The PromptOS checklist passes 19/19, the authenticated gate passes 6/6, and the public gate passes 8/8 across desktop/mobile and 1440/768/390/320px with runtime, axe, overflow, console and screenshot checks. All 36 current-source snapshots at exact 1440x1024 and 390x844 viewports were manually reviewed. Current repository checkpoint: 184 files / 1,371 tests passed. Current source-contract and 14-route authenticated subset pass; full PromptOS route/state/pixel-parity acceptance remains open, while staging visual acceptance remains 18/20 because the deployed Worker predates `[data-cart-variant-id]`. Neither artifact authorizes staging or production mutation. To intentionally update the reviewed baselines after a UI change, run the corresponding local browser gate with `--update-snapshots` and inspect every changed PNG.
