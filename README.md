...
`platform:route-preflight` là checkpoint staging-only, read-only: nó dùng riêng `CLOUDFLARE_ROUTE_AUDIT_API_TOKEN`, pin Wrangler vào account đã khai báo, đối chiếu live D1 name+UUID và gọi đúng một Worker Routes `GET`; token và raw route response không được in hoặc truyền vào child process. Checkpoint này không thay thế full `platform:doctor`, vốn vẫn cần cả `CLOUDFLARE_PLATFORM_API_TOKEN` và `CLOUDFLARE_ROUTE_AUDIT_API_TOKEN` trước khi chạy backup thật, migration hoặc deploy staging. Source/local chain hiện tới `0077`; staging ledger được chấp nhận vẫn dừng ở `0028`, nên đúng 49 migration `0029`-`0077` còn pending. Read-only evidence hiện tại xác nhận staging D1 identity và `MEDIA` đang private, nhưng phải kiểm tra lại route inventory, D1 identity và `MEDIA` ngay trước mọi mutation; backup report-v2 của đúng staging D1 cũng phải mới không quá 60 phút. Xem thứ tự mutation có guard tại `docs/RELEASE.md`; không suy diễn deploy/migration đã xảy ra từ dry-run.
