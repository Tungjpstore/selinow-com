...
Seller sessions remain opaque and D1-authoritative. `GET /api/auth/sessions` returns only session IDs and lifecycle timestamps, marks the current session and excludes token hashes. Authenticated activity refreshes `last_seen_at` at a bounded interval without making that metadata part of admission. Sensitive seller routes require an active, unexpired session plus their existing CSRF, role, tenant, idempotency, version and confirmation guards; they do not impose an additional authentication-age window. `DELETE /api/auth/sessions` revokes every active session for the authenticated user and clears the current browser cookies. A revoked session cannot be restored by a later activity update.
