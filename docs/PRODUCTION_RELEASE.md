...
`--phase promote` is pinned to `infra/release/production-promotion-staging.json`. That checked-in release contract must remain an exact derivation of `infra/environments/staging.json` with only `sharedZoneDisabledRoutes` changed to an empty array. Both the planner and executor verify this relationship before admission and reject a `--staging-spec` override that differs from the canonical promotion contract. The normal staging deployment spec retains those operator-managed patterns, while live staging admission accepts only a complete pre-handoff null-guard pair or the complete approved production handoff pair; mixed or unknown ownership still fails closed.
