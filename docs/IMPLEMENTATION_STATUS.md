...
- Read-only Cloudflare Dashboard inspection confirmed seven shared-zone Worker routes: `staging.selinow.com/*`, `app-staging.selinow.com/*`, `api-staging.selinow.com/*`, `*.staging.selinow.com/*` and `*/*` point to `selinow-com-staging`; `selinow.com/*` and `*.selinow.com/*` point to `selinow-com-production`.
- This live inventory does not satisfy the checked-in staging mutation contract, which requires the apex and wildcard entries to remain exact `script=null` guards. The discrepancy is recorded as a route-contract blocker; no route, Worker, D1, DNS or production mutation was performed.
