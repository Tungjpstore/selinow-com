...
- Real staging mutation is fail-closed behind shared admission: backup, migrate, seed and deploy require the shared-zone pair to be either the complete pre-handoff `script=null` guards or the complete approved production handoff, `*.staging.selinow.com/*` and `*/*` to point to `selinow-com-staging`, exact account/runtime/live-D1 identity and immediate rechecks. Migrate, seed and deploy additionally require fresh report-v2 backup evidence for the admitted D1 target before their final sink; direct commands cannot bypass the gate. Build-only and dry-run packaging remain offline.
