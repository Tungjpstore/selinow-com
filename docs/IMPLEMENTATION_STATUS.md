...
- Regression coverage proves the no-window default no longer rejects an older active session while explicitly requested age windows still fail with `recent_auth_required`. Focused auth, high-risk route and rollback suites (21 tests), TypeScript, lint and `git diff --check` pass for the implementation lineage beginning at `ab92eac`.
