...
- Before the ceremony, staging admission was correctly blocked by the dirty tree, missing scoped tokens, and stale manifests ending at `0113`; those blockers were cleared using the clean commit and temporary operator token context. No stale manifest was reused.
- Staging release `stg_20260823T031316Z_f3d96bf0f684` is now complete for this candidate: pre-migration backup `bkp_20260823031141_4dbae68ba270`, restore drill `rdr_20260823031211_e11e84cd21cd`, migrations `0114` and `0115`, post-migration backup `bkp_20260823031447_e095ecd5530d`, restore drill `rdr_20260823031523_a15f37544f1e`, and `db:complete-release` all passed. Worker version `40d37754-49e1-404d-9975-294bffb391f1` deployed at 100%; immutable evidence is bound at `.wrangler/releases/staging/stg_20260823T031316Z_f3d96bf0f684/deployment-evidence.json` with artifact SHA-256 `223c167116614a2c1b0599fa07228295369b4c2aa236d2b96b7f8d3f9e5ee09a`.
- Post-deploy staging verification passed: migration status has no pending rows, D1 preflight, platform doctor, route preflight, trigger inventory, and the 8/8 Phase-A smoke checks are green. Direct D1 read for `shop_603fa8eb-b753-4815-9c1e-7b1931df61fe` shows `pro`, `active`, period end `2026-09-23T00:15:57.457058Z`, `premiumStorefrontTemplates=true`, and `analytics=advanced`; the index `idx_orders_shop_paid_julianday_id` exists.
- Browser UAT after deploy shows Pulse and Desk radios visible and enabled, the Overview exposes the 90-day revenue window, pricing renders Starter `99,000 ₫` and Pro `299,000 ₫`, `/onboarding?plan=pro` hands an existing shop to billing, and the same-plan target shows an explicit disabled current-Pro card rather than a downgrade selection. Existing test-mode checkout `bchk_6b293b3c-1b00-4bc3-a0de-90526f565cba` remains completed with an active Dodo subscription; the signed `payment.succeeded` event is processed.
- The optional visual suite reports 12 unrelated pre-existing storefront snapshot/accessibility failures (8/20 pass): `aria-hidden-focus` on storefront product marks and large snapshot drift in public storefront/login fixtures. Billing/entitlement UAT is green; these storefront baseline issues remain documented and do not alter the billing release evidence.
