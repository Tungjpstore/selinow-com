...
- Real staging mutation is fail-closed behind shared admission: backup, migrate, seed and deploy require `selinow.com/*` and `*.selinow.com/*` to point to `selinow-com-production`, `*.staging.selinow.com/*` and `*/*` to point to `selinow-com-staging`, exact account/runtime/live-D1 identity and immediate rechecks. Null guards, deleted legacy staging routes, bare-host route entries and any other extra binding fail closed. Migrate, seed and deploy additionally require fresh report-v2 backup evidence for the admitted D1 target before their final sink; direct commands cannot bypass the gate. Build-only and dry-run packaging remain offline.
