...
- Preserved the explicit five-minute step-up for the four platform-risk endpoints that opt into it: payment-appeal decisions, dead-letter operations, deletion legal-hold changes and live key-rotation processing.
...
- Regression coverage proves the default 15-minute call no longer rejects an older active session while an explicit five-minute call still fails with `recent_auth_required`. Focused auth, high-risk route and rollback suites (21 tests), TypeScript, lint and `git diff --check` pass for the implementation at `ab92eac`.
