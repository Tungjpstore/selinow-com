...
Environment identity remains separate from runtime activation. Staging has the accepted 28-migration runtime, with source migrations `0029`-`0069` still pending behind read-only and fresh-backup gates. Production has reviewed D1/R2/KV/Queue/Turnstile resource identity, an admitted Worker/platform handoff and the applied `0001`-`0052` D1 baseline; continuation migrations `0053`-`0069` remain pending. Non-secret identifiers are intentionally pinned in `infra/environments/production.json`, `infra/generated/production.json` and `wrangler.jsonc` for fail-closed admission. Provider activation and full commerce traffic remain separate NO-GO gates.
...
The current bounded surfaces are `GET /api/v1/shop` (`shop:read`), `GET /api/v1/catalog` (`catalog:read`), `GET /api/v1/inventory` (`inventory:read`) and `GET /api/v1/orders` (`orders:read`). They return minimal tenant-derived projections; inventory exposes aggregate counts and orders expose redacted summaries, while payment, fulfillment, entitlement and webhook operations remain unauthorized. Further scopes require an explicit resource/operation matrix, tenant-isolation and concurrency coverage, lifecycle/export/deletion updates and a reviewed expansion of the credential grant model; client-provided tenant overrides remain invalid.

Migration `0069_catalog_channel_visibility.sql` adds an explicit tenant/product/channel
visibility ledger. A missing row is hidden by default; Website and enabled-channel defaults
are backfilled, while seller mutations remain optimistic, idempotent and capability-gated.
The ledger is a catalog projection fence, not provider activation or payment/fulfillment
authority.
