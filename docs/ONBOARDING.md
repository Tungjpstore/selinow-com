...
Seller onboarding is browser-first, resumable and stored server-side. The only unavoidable external inputs are a Telegram bot token, PayOS credentials and DNS records for an optional custom domain.
