...
The accepted staging ledger still ends at `0028_domain_delivery_runtime_hardening.sql` with exactly 49 pending source migrations, `0029` through `0077`; no staging mutation has been performed. `db:preflight` remains the read-only source of truth for the remote ledger and reports `payment_provider_projection=not_applied` until the continuation chain is admitted. The current final-tree staging packaging dry-run passes with 279 modules and approximately 3,843.40 KiB upload and exits before Wrangler deployment. The route preflight remains fail-closed with `cloudflare_route_audit_api_token_missing` when the temporary audit token is absent. No request body, checkout/payment call, staging migration, seed, deploy or production mutation was performed; production remains `NO-GO` for full commerce/provider activation.
...
- Staging has not received the Phase A source slice. The accepted remote ledger remains at `0028`; migrations `0029`-`0077` are still pending behind live-route, exact account/D1 and fresh report-v2 backup admission. Do not describe Phase A or later continuation slices as live until post-migration status and smoke evidence prove them.
...
- The Phase A schema additions are `0030_order_checkout_cart_reference.sql` and `0033_cart_mutation_replays.sql`, but the current broad Worker is not a valid partial-schema artifact: storefront resolution requires `0029`, the scheduled worker unconditionally purges `0041` claim rows, and Telegram runtime/outbox reads `0045` locale columns without a fallback. Payment readiness, API credentials, exports and later fulfillment paths also read post-Phase-A tables directly. The supported current-tree cutover is therefore the complete ordered `0029`-`0077` chain; a true Phase-A-only rollout needs a separately scoped Worker/source and migration design.
...
The latest retained local report is `.wrangler/restore-drills/local/rdr_20260802132434_f77680c70c88.json`: it applies the prior `0001`-`0066` source chain, restores 612 items, passes integrity/FK/schema/count checks, records the known alias normalization in the disposable copy and leaves the authoritative local D1 unchanged. A fresh disposable `0001`-`0077` SQLite apply also passes integrity and foreign-key checks with a 77-row ledger; this remains local-only evidence and is not a protected report-v2 staging/production backup or external release evidence.
...
Migrations `0049`-`0052` are present in the admitted production baseline but remain staging-pending. Before any staging deployment that reads these tables, require the exact migration status, live-route/account/D1 admission and fresh report-v2 backup sequence; apply the complete ordered chain through `0077`, then verify all eight generated-license tables, the activation milestone ledger, tenant-leading indexes, same-tenant foreign keys, immutable request transitions, scheduler/key-version indexes and rotation resource types. Never deploy dependent code while staging remains at `0028`.
