...
Staging D1 is observed through `0120`; source continues through `0121`. Every new candidate must still freshly admit that exact ordered ledger, complete its post-migration evidence and create a new immutable deployment binding; historical manifests and deployment artifacts are not reusable. `db:migrate:status`, `db:preflight` and the post-migration schema contract are the read-only sources of truth for the remote ledger. Worker deploy and provider UAT remain separate gates. The route preflight remains fail-closed with `cloudflare_route_audit_api_token_missing` when the temporary audit token is absent. Production remains through `0112`, and no `0113`-`0121` production migration or provider activation is claimed; production remains `NO-GO` for full commerce/provider activation.
...
- Staging D1 is observed through `0120`; source migration `0121` remains unaccepted remotely until the exact candidate has its own manifest, post-migration evidence, immutable deployment identity, health, route and provider evidence.
...
- The current broad Worker is not valid evidence for a different source candidate: storefront resolution, scheduled cleanup, Telegram runtime/outbox, Dodo checkout reconciliation, Pro entitlements, auth admission and PayOS provider projection depend on the complete schema/runtime pair. The supported reviewed cutover is the complete source chain through `0121`; provider activation still requires the combined payment/non-payment handoff.
...
Migrations `0049`-`0052` are present in the admitted production baseline. Before any staging deployment that reads current source tables or guards, require the exact migration status, live-route/account/D1 admission and two-phase report-v2 backup/restore sequence; verify the complete ordered chain through `0121`, including generated-license, activation, recovery, privacy, platform-admin, PayOS claim-fencing and provider projection, buyer order recovery, custom-domain Turnstile admission, shop-creation admission, Dodo checkout reconciliation, Pro entitlements, auth admission tables/guards, disconnect/reconnect identity history and stale projection repair. Require tenant-leading indexes, same-tenant foreign keys, immutable request transitions, scheduler/key-version indexes and rotation resource types. Never deploy dependent code before `db:complete-release`, the post-migration contract and smoke gates pass.
