...
Staging D1 is observed through the complete `0001`-`0119` source ledger. Every new candidate must still freshly admit that exact ordered ledger, complete its post-migration evidence and create a new immutable deployment binding; historical manifests and deployment artifacts are not reusable. `db:migrate:status`, `db:preflight` and the post-migration schema contract are the read-only sources of truth for the remote ledger. Worker deploy and provider UAT remain separate gates. The route preflight remains fail-closed with `cloudflare_route_audit_api_token_missing` when the temporary audit token is absent. Production remains through `0112`, and no `0113`-`0119` production migration or provider activation is claimed; production remains `NO-GO` for full commerce/provider activation.
...
- Staging D1 is applied through `0119`, but do not describe a Worker or provider UAT as accepted until the exact candidate has its own manifest, post-migration evidence, immutable deployment identity, health, route and provider evidence.
...
- The current broad Worker is not valid evidence for a different source candidate: storefront resolution, scheduled cleanup, Telegram runtime/outbox, Dodo checkout reconciliation, Pro entitlements, auth admission and PayOS provider projection depend on the complete schema/runtime pair. The supported reviewed cutover is the complete source chain through `0119`; provider activation still requires the combined payment/non-payment handoff.
