...
- **Acceptance:** Require the authenticated account and exact live D1 name+UUID to match the checked-in staging identity; `selinow.com/*` and `*.selinow.com/*` must be the unique pre-handoff `script=null` guard pair or the exact approved `selinow-com-production` handoff pair; `*.staging.selinow.com/*` and `*/*` must point only to `selinow-com-staging`; malformed, duplicate or extra bindings fail closed.
...
2. Stop without mutation unless the doctor proves the authenticated Wrangler account matches `infra/environments/staging.json`, the SaaS DNS/fallback contract is ready, the shared-zone boundary is either the exact pre-handoff `script=null` pair or the exact approved production handoff pair, and both `*.staging.selinow.com/*` and `*/*` point only to `selinow-com-staging`. Shared mutation admission additionally requires the live D1 list to contain exactly the generated-manifest database name and UUID. Missing credentials, unreadable inventory or any drift is a failed admission.
3. After an approved staging change window, create the protected staging backup, run `npm run restore:drill -- --env staging --reviewed-commit "$(git rev-parse HEAD)" --json`, then write the candidate-bound manifest with `npm run release:staging:manifest -- --write --json`. Use the exact emitted `manifestRef` for `db:migrate`, optional `db:seed`, and `deploy:staging` via `--release-manifest`; commands without it fail closed. The manifest binds the clean commit/tree, exact live ledger prefix, backup checksum/target and isolated restore report. Real staging mutation commands repeat account, live D1 and route admission immediately before Wrangler, and deploy repeats them after the build. `db:migrate` applies every pending numbered migration in filename order; for the current ledger that is the complete `0029`-`0080` chain, not a Phase A-only subset. Review the whole pending chain before opening the window and never deploy current source against a partially applied schema.
...
The accepted staging ledger still ends at `0028_domain_delivery_runtime_hardening.sql` with exactly 52 pending source migrations, `0029` through `0080`; no staging mutation has been performed. `db:preflight` remains the read-only source of truth for the remote ledger and reports `payment_provider_projection=not_applied` until the continuation chain is admitted. The current final-tree staging packaging dry-run passes and exits before Wrangler deployment. The route preflight remains fail-closed with `cloudflare_route_audit_api_token_missing` when the temporary audit token is absent. No request body, checkout/payment call, staging migration, seed, deploy or production mutation was performed; production remains `NO-GO` for full commerce/provider activation.
...
- Staging has not received the Phase A source slice. The accepted remote ledger remains at `0028`; migrations `0029`-`0080` are still pending behind live-route, exact account/D1 and fresh report-v2 backup admission. Do not describe Phase A or later continuation slices as live until post-migration status and smoke evidence prove them.
...
- The Phase A schema additions are `0030_order_checkout_cart_reference.sql` and `0033_cart_mutation_replays.sql`, but the current broad Worker is not a valid partial-schema artifact: storefront resolution requires `0029`, the scheduled worker unconditionally purges `0041` claim rows, and Telegram runtime/outbox reads `0045` locale columns without a fallback. Payment readiness, API credentials, exports and later fulfillment paths also read post-Phase-A tables directly. The supported current-tree cutover is therefore the complete ordered `0029`-`0080` chain; a true Phase-A-only rollout needs a separately scoped Worker/source and migration design.
...
The retained local report `.wrangler/restore-drills/local/rdr_20260802132434_f77680c70c88.json` applies the prior `0001`-`0066` source chain, restores 612 items, passes integrity/FK/schema/count checks, records the known alias normalization in the disposable copy and leaves the authoritative local D1 unchanged. The current candidate-bound local restore report applies the contiguous `0001`-`0080` chain and passes integrity and foreign-key checks with an 80-row ledger; this remains local-only evidence and is not a protected report-v2 staging/production backup or external release evidence.
...
Migrations `0049`-`0052` are present in the admitted production baseline but remain staging-pending. Before any staging deployment that reads these tables, require the exact migration status, live-route/account/D1 admission and fresh report-v2 backup sequence; apply the complete ordered chain through `0080`, then verify all eight generated-license tables, the activation milestone ledger, catalog activation timestamps, tenant-leading indexes, same-tenant foreign keys, immutable request transitions, scheduler/key-version indexes and rotation resource types. Never deploy dependent code while staging remains at `0028`.
