...
- **Acceptance:** Require the authenticated account and exact live D1 name+UUID to match the checked-in staging identity; `selinow.com/*` and `*.selinow.com/*` must point only to `selinow-com-production`, while `*.staging.selinow.com/*` and `*/*` must point only to `selinow-com-staging`; malformed, duplicate, missing or extra bindings fail closed.
...
2. Stop without mutation unless the doctor proves the authenticated Wrangler account matches `infra/environments/staging.json`, the SaaS DNS/fallback contract is ready, the shared-zone apex/wildcard pair points only to `selinow-com-production`, and both `*.staging.selinow.com/*` and `*/*` point only to `selinow-com-staging`. Shared mutation admission additionally requires the live D1 list to contain exactly the generated-manifest database name and UUID. Missing credentials, unreadable inventory or any drift is a failed admission.
3. After an approved staging change window, create the protected staging backup, run `npm run restore:drill -- --env staging --reviewed-commit "$(git rev-parse HEAD)" --json`, then write the candidate-bound manifest with `npm run release:staging:manifest -- --write --json`. Use the exact emitted `manifestRef` for `db:migrate --confirm-maintenance-drain`, optional `db:seed`, and `deploy:staging` via `--release-manifest`; commands without it fail closed. The manifest binds the clean commit/tree, exact live ledger prefix, backup checksum/target and isolated restore report. Real staging mutation commands repeat account, live D1 and route admission immediately before Wrangler, and deploy repeats them after the build. `db:migrate` applies every pending numbered migration in filename order; for the current tree that is the complete ordered chain through `0083_auth_admission_subject_limits.sql`, not a Phase A-only subset. Review the whole pending chain before opening the window and never deploy current source against a partially applied schema.
...
- The Phase A schema additions are `0030_order_checkout_cart_reference.sql` and `0033_cart_mutation_replays.sql`, but the current broad Worker is not a valid partial-schema artifact: storefront resolution requires `0029`, the scheduled worker unconditionally purges `0041` claim rows, and Telegram runtime/outbox reads `0045` locale columns without a fallback. Payment readiness, API credentials, exports and later fulfillment paths also read post-Phase-A tables directly. The supported reviewed cutover remains the complete `0029`-`0080` chain; candidate migrations `0081`-`0083` require the combined payment/non-payment handoff before they can extend that operational contract.
