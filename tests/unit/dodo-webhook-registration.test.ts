...
describe("Dodo webhook registration", () => {
  ...
  it.each(["payment.cancelled", "payment.processing", "subscription.paused", "subscription.unpaused", "subscription.update_payment_method"])("fails closed when an existing endpoint omits required event %s", async (missingEvent) => {
    const fetcher = vi.fn()
      .mockResolvedValueOnce(Response.json({ items: [{
