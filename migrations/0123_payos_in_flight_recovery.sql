...
-- A disconnected integration can retain an in-flight claim when a Worker
-- invocation is interrupted after the claim write. Release only that stale
-- non-active claim so the next owner-initiated reconnect can establish a new
-- fenced claim.
...
WHERE provider = 'payos'
  ...
  AND provider_claim_state IN ('in_flight', 'quarantined')
  AND provider_claim_nonce IS NOT NULL;
