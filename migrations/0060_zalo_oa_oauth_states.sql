...
-- OAuth state is a short-lived tenant/request envelope. Raw browser state and
-- PKCE verifiers never enter D1; only the HMAC state hash and encrypted
-- verifier are retained until one-use consumption or explicit revocation.
CREATE TABLE channel_oauth_states (
  ...
  UNIQUE (shop_id, connector_request_id, provider_code),
...
-- State may only be issued for an active tenant with a live subscription. The
-- application still performs the same admission check before writing state.
...
-- A pending state can be consumed or revoked once. Identity, ciphertext and
-- timestamps are immutable; versioned CAS updates prevent double consumption.
