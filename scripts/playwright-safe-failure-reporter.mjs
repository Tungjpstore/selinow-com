...
export function redactPlaywrightFailure(value) {
  return String(value)
    ...
    .replace(/((?:SESSION_SECRET|MAGIC_LINK_SECRET|IDENTIFIER_HMAC_SECRET|CREDENTIAL_KEK_V1|INVENTORY_KEK_V1|EXPORT_KEK_V1|TURNSTILE_SECRET_KEY|GOOGLE_OAUTH_CLIENT_ID|GOOGLE_OAUTH_CLIENT_SECRET|DODO_PAYMENTS_API_KEY|DODO_PAYMENTS_WEBHOOK_KEY|DODO_PAYMENTS_WEBHOOK_SECRET|DODO_API_KEY|DODO_WEBHOOK_SECRET|PAYOS_STAGING_CHANNEL_IDENTITY_FINGERPRINT|PAYOS_CONTROLLED_STAGING_CLIENT_ID|CLOUDFLARE_API_TOKEN|CLOUDFLARE_D1_API_TOKEN|CLOUDFLARE_WORKER_DEPLOY_API_TOKEN|CLOUDFLARE_PLATFORM_API_TOKEN|CLOUDFLARE_ROUTE_AUDIT_API_TOKEN|CLOUDFLARE_STAGING_RESOURCE_AUDIT_API_TOKEN|CLOUDFLARE_STAGING_DEPLOYMENT_AUDIT_API_TOKEN|CLOUDFLARE_STAGING_TRIGGER_AUDIT_API_TOKEN|CLOUDFLARE_PRODUCTION_BOOTSTRAP_MIGRATION_API_TOKEN|CLOUDFLARE_PRODUCTION_EMPTY_BASELINE_API_TOKEN|CLOUDFLARE_OAUTH_TOKEN|CF_API_TOKEN)\s*[:=]\s*)[^\s,;]+/gu, "$1[redacted]")
