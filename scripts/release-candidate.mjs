...
try {
  ...
  if (!options.execute) {
    ...
    const auditEnvironment = buildCanaryWranglerEnvironment(operatorEnvironment, productionSpec.accountId, auditToken);
    const workerEnvironment = buildCanaryWranglerEnvironment(operatorEnvironment, productionSpec.accountId, workerToken);
    const liveInput = {
      auditEnvironment,
      auditToken,
    ...
    const artifactSha256 = await fingerprintProductionUploadInputs(repositoryRoot);
    runWrangler([
      "versions", "upload", "--env", "production", "--strict",
      "--tag", tag,
    ...
    const postUploadArtifactSha256 = await fingerprintProductionUploadInputs(repositoryRoot);
    if (
    ...
    const after = await discoverLiveState(liveInput);
    ...
    const candidateWorkerVersion = captureProductionCandidateVersion({
    ...
    const bindingNames = validateProductionCandidateVersionView(candidateView, candidateWorkerVersion, {
      generatedManifest,
      productionSpec,
      wranglerConfig,
    });
