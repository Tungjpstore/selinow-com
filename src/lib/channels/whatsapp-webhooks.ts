...
async function extractEventDescriptor(rawBody: Uint8Array): Promise<{ action: string; eventId: string }> {
  ...
  for (const rawEntry of payload.entry) {
    ...
    for (const rawChange of changes) {
      if (typeof rawChange !== "object" || rawChange === null || Array.isArray(rawChange)) continue;
      ...
      if (Array.isArray(record.statuses)) {
        hasStatus = true;
        for (const rawStatus of record.statuses) {
  ...
  if (uniqueIds.length === 1 && SAFE_EVENT_ID.test(uniqueIds[0] ?? "")) {
    eventId = uniqueIds[0] ?? `batch_${digest}`;
...
export async function loadWhatsAppWebhookContext(env: AppBindings, connectionPublicId: string): Promise<WhatsAppWebhookContext> {
  ...
  const row = await env.PLATFORM_DB.prepare(`
    ...
    INNER JOIN shop_subscriptions AS subscription
      ...
      AND subscription.state IN ('trialing', 'active', 'past_due')
...
export async function processWhatsAppWebhook(input: {
  ...
  const descriptor = await extractEventDescriptor(rawBody);
