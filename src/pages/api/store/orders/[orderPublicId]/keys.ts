...
export const GET: APIRoute = async ({ locals, params, request }) => {
  try {
    ...
    const result = await createWebsiteCommerceApplication(env, shop).revealFulfillment({
    ...
    const fulfillment = { keys: result.items, orderId: result.orderId };
    return Response.json({ fulfillment, ok: true, requestId: locals.requestId }, { headers: { "Cache-Control": "private, no-store, max-age=0", "Pragma": "no-cache", "X-Robots-Tag": "noindex, nofollow" } });
