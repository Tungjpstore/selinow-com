...
import { cancelAutomationTask } from "../../../../../../../lib/automation/api-service";
...
function parseCancelBody(body: Record<string, unknown>): {
  expectedVersion: number;
  reasonCode: string;
} {
  rejectUnknownFields(body, ["expectedVersion", "reasonCode"]);
  if (typeof body.reasonCode !== "string") {
    throw new AppError("validation_failed", 400, ["reason_code_required"]);
  }
  return {
...
export const POST: APIRoute = async ({ locals, params, request }) => {
  try {
    ...
    const result = await cancelAutomationTask({
      ...parseCancelBody(await readJsonObject(request, 4 * 1_024)),
      env,
