...
import { getPublicApiInventory } from "../../../lib/api/inventory";
...
export const GET: APIRoute = async ({ locals, request }) => {
  try {
    ...
    const context = await authenticatePublicApiRequest({ env, request, requiredScope: "inventory:read" });
    const page = parsePublicApiPage(new URL(request.url));
    const lowStock = await env.PLATFORM_DB.prepare(`
      SELECT low_stock_threshold AS lowStockThreshold
      FROM shop_settings
      WHERE shop_id = ?
      LIMIT 1
    `).bind(context.shopId).first<{ lowStockThreshold: number }>();
    const inventory = await getPublicApiInventory({
      env,
      lowStockThreshold: lowStock?.lowStockThreshold ?? 0,
      page,
      shopId: context.shopId,
    });
    await recordPublicApiUsage({ context, env, requestId: locals.requestId });
    return Response.json({ data: { inventory, shop: context.shop }, ok: true, requestId: locals.requestId }, {
      headers: {
